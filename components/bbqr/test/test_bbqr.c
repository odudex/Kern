...
void test_bbqr_binary_file_type(void) {
  ...
  if (part.encoding != BBQR_ENCODING_HEX ||
      part.file_type != BBQR_TYPE_BINARY || part.total != 1 ||
      part.index != 0 || part.payload_len != 4 ||
...
void test_bbqr_hex_binary_encode(void) {
  ...
  uint8_t *decoded = bbqr_decode_payload(BBQR_ENCODING_HEX, payload,
