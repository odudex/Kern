...
#define BAT_ADC_ATTEN ADC_ATTEN_DB_12 /* 4.2 V pack lands at 1.4 V on the pin */
#define BAT_ADC_SAMPLES 16            /* the divider node is a noisy source */
...
esp_err_t bsp_pmic_init(void) {
  ...
  ESP_RETURN_ON_ERROR(adc_oneshot_config_channel(adc, BAT_ADC_CHANNEL, &chan),
                      TAG, "channel");
  return adc_cali_create_scheme_curve_fitting(&cal, &cali);
}

esp_err_t bsp_pmic_get_battery_mv(uint16_t *mv) {
  int raw, sum = 0, node_mv;
  if (!cali || !mv)
    return ESP_ERR_NOT_SUPPORTED;
  for (int i = 0; i < BAT_ADC_SAMPLES; i++) {
    ESP_RETURN_ON_ERROR(adc_oneshot_read(adc, BAT_ADC_CHANNEL, &raw), TAG,
                        "read");
    sum += raw;
  }
  ESP_RETURN_ON_ERROR(
      adc_cali_raw_to_voltage(cali, sum / BAT_ADC_SAMPLES, &node_mv), TAG,
      "convert");
  *mv = node_mv * BAT_DIVIDER_MUL;
  return ESP_OK;
}
