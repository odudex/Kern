...
/* The CrowPanel display panel requires a separate 3.3 V supply (LDO channel 4).
   Acquire it once and keep it for the lifetime of the application. */
static esp_err_t bsp_enable_panel_power(void) {
  static esp_ldo_channel_handle_t panel_pwr_chan = NULL;
  if (panel_pwr_chan) {
    return ESP_OK;
  }
  esp_ldo_channel_config_t ldo_cfg = {
      .chan_id = BSP_PANEL_PWR_LDO_CHAN,
      .voltage_mv = BSP_PANEL_PWR_LDO_VOLTAGE_MV,
  };
  ESP_RETURN_ON_ERROR(esp_ldo_acquire_channel(&ldo_cfg, &panel_pwr_chan), TAG,
                      "Acquire LDO channel for panel power failed");
  ESP_LOGI(TAG, "Panel power rail (%d mV) enabled",
           BSP_PANEL_PWR_LDO_VOLTAGE_MV);
  return ESP_OK;
}
...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config,
  ...
  ESP_RETURN_ON_ERROR(bsp_enable_panel_power(), TAG, "Panel power failed");
  ...
  ESP_GOTO_ON_ERROR(esp_lcd_new_panel_ek79007(io, &lcd_dev_config, &disp_panel),
...
lv_display_t *bsp_display_start(void) {
  ...
  // Increase the adapter task stack from its 4 KB default: libwally descriptor
  // parsing has deep call chains.
