/**
 * Compatibility shim: provides legacy mbedtls_sha512_* functions using PSA
 * crypto API.
 *
 * When CONFIG_MBEDTLS_HARDWARE_SHA is enabled on ESP32-P4, ESP-IDF v6.x
 * undefines MBEDTLS_SHA512_C (replacing it with PSA hardware acceleration).
 * This removes the legacy mbedtls_sha512_init/starts/update/finish/free symbols
 * that libwally-core calls directly. This file re-implements them on top of
 * psa_hash_*.
 */
...
/* Only needed when hardware SHA is enabled (which removes MBEDTLS_SHA512_C) */
    ...
    defined(CONFIG_SOC_SHA_SUPPORT_SHA512)
 ...
 */
_Static_assert(sizeof(psa_hash_operation_t) <=
                   sizeof(((mbedtls_sha512_context *)0)->buffer),
...
int mbedtls_sha512_finish(mbedtls_sha512_context *ctx, unsigned char *output) {
  ...
  size_t actual_len = 0;
  psa_status_t status =
      psa_hash_finish(get_op(ctx), output, hash_len, &actual_len);
  return (status == PSA_SUCCESS) ? 0 : -1;
}
