...
esp_err_t bsp_display_new_with_handles(const bsp_display_config_t *config,
  ...
  /* Stock EK79007 1024x600@60Hz timings (52 MHz DPI clock), as used by the
     Waveshare upstream BSP for this panel. */
#if CONFIG_BSP_LCD_COLOR_FORMAT_RGB888
  esp_lcd_dpi_panel_config_t dpi_config =
      EK79007_1024_600_PANEL_60HZ_CONFIG_CF(LCD_COLOR_FMT_RGB888);
#else
  esp_lcd_dpi_panel_config_t dpi_config =
  ...
  /* The panel is mounted rotated 180 degrees on this board; flip both scan
     directions (Waveshare's BSP applies the same transform). */
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config,
  ...
  const esp_lcd_touch_config_t tp_cfg = {
      ...
      .flags =
          {
              /* Match the 180-degree panel mirror applied in
