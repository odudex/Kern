...
- PSRAM encryption is enabled automatically and protects runtime key material in external RAM.
...
**The point of no return, and only after the Phase 4 SD update path has been re-validated under secure boot (Phase 6).** This is the single unrecoverable step; everything before it allowed serial recovery.
