...
static int32_t bip138_aead_encrypt(void *ctx, const uint8_t *key,
  ...
  psa_status_t status = psa_aead_encrypt(
      key_id, PSA_ALG_CHACHA20_POLY1305, nonce, BIP138_NONCE_LEN, NULL, 0,
      plaintext, plaintext_len, out, out_cap, out_len);
...
static int32_t bip138_aead_decrypt(void *ctx, const uint8_t *key,
  ...
  psa_status_t status = psa_aead_decrypt(
      key_id, PSA_ALG_CHACHA20_POLY1305, nonce, BIP138_NONCE_LEN, NULL, 0,
      ciphertext, ciphertext_len, out, out_cap, out_len);
  psa_destroy_key(key_id);
  return status == PSA_SUCCESS ? 0 : -1;
}
