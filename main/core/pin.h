...
#define PIN_MIN_MAX_FAILURES 5
#define PIN_MAX_MAX_FAILURES 50
 ...
 */
KERN_WARN_UNUSED_RESULT esp_err_t pin_init(void);
...
KERN_WARN_UNUSED_RESULT esp_err_t pin_efuse_provision(void);
 ...
 */
KERN_WARN_UNUSED_RESULT esp_err_t pin_compute_anti_phishing(
    const char *prefix, size_t len, const char **word1_out,
    const char **word2_out, uint8_t identicon_out[3]);

/* PIN lifecycle. To change a PIN, verify the current one via pin_verify(),
 * then call pin_setup() with the new PIN. */
KERN_WARN_UNUSED_RESULT bool pin_is_configured(void);
KERN_WARN_UNUSED_RESULT esp_err_t pin_setup(const char *pin, size_t len,
                                            uint8_t split_pos);
pin_verify_result_t pin_verify(const char *pin, size_t len);

/* Remove PIN without wiping user data (for "Disable PIN" in settings) */
KERN_WARN_UNUSED_RESULT esp_err_t pin_remove(void);

/* Config getters/setters */
KERN_WARN_UNUSED_RESULT uint8_t pin_get_split_position(void);
KERN_WARN_UNUSED_RESULT uint32_t pin_get_delay_ms(void);
KERN_WARN_UNUSED_RESULT uint8_t pin_get_fail_count(void);
KERN_WARN_UNUSED_RESULT uint8_t pin_get_max_failures(void);
KERN_WARN_UNUSED_RESULT bool pin_has_anti_phishing(void);
KERN_WARN_UNUSED_RESULT esp_err_t pin_set_max_failures(uint8_t max);

/* Nuclear wipe: erase "pin" NVS + settings + SPIFFS.
 * Called when max failures reached. Ends in esp_restart() and does not return,
 * so there is no status for a caller to act on; each step logs what it could
 * not complete. */
