...
static bool registry_save_bip138(const char *id, const char *descriptor_str,
  ...
  const char *err = NULL;
  int32_t ret = bip138_encrypt(
      kern_bip138_crypto_vtable(), key, 1, NULL, 0, BIP138_CONTENT_BIP380, 0,
      NULL, 0, (const uint8_t *)descriptor_str, strlen(descriptor_str),
      BIP138_PADDING_NONE, &encrypted, &err);
  if (ret != BIP138_OK) {
...
static bool registry_decrypt_bip138(const uint8_t *encrypted,
...
static bool descriptor_id_from_filename(storage_location_t loc,
...
static void registry_init_scan(storage_location_t loc) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (desc_str) {
      ...
      if (!descriptor_id_from_filename(loc, fname,
                                       encrypted ? STORAGE_DESCRIPTOR_EXT_KEF
