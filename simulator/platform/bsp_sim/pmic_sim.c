...
#elif defined(BSP_BATTERY_ADC)

#include <stdlib.h>

/*
 * Simulated BAT_ADC divider for boards with no PMIC (wave_43, wave_5,
 * wave_7b). Voltage is all the hardware can read, so percentage and charge
 * status stay unsupported, exactly as on device.
 */
...
/* KERN_SIM_BATTERY_MV drives the low-battery warning without a rebuild. */
esp_err_t bsp_pmic_get_battery_mv(uint16_t *mv) {
  if (!pmic_initialized)
    return ESP_ERR_INVALID_STATE;
  const char *override = getenv("KERN_SIM_BATTERY_MV");
  *mv = override ? (uint16_t)atoi(override) : 3800;
  return ESP_OK;
}
